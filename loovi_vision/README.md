...
loovi_vision/
  ...
  enrich/
    ...
    summary.py         # 세션 요약(주목률, 성별·연령 분포, per_track) + demographics_of
  analysis/
...
설정(`loovi_vision/configs/person_only.yaml`):

```yaml
models:
  face_pack: buffalo_l
face:
  enable: true               # false 면 기존 person_only 동작과 100% 동일
  conf_min: 0.5
  run_every_n_frames: 3      # track 당 얼굴 검출 주기 (속도)
...
gaze:
  ...
  grade_view_sec: 1.0        # 시청시간 분포 경계: under_1s / 1_to_2s / over_2s
...
`dwell_distribution`(LTS 인원의 시청시간 분포: **1~2초 / 2초 이상** 2구간).
...
{ "device_id":"...", "board_id":"...", "seq":1,
  ...
  "ots_demographics":{ "gender":{"male":4,"female":3},
                       "male_age":{"20s":2,"...":0}, "female_age":{"20s":2,"...":0} },
  "lts_demographics":{ "gender":{"male":2,"female":1},
                       "male_age":{"20s":1,"...":0}, "female_age":{"20s":1,"...":0} },
  "attention":{ "avg_dwell_sec":1.6, "dwell_sum_sec":4.8, "dwell_distribution":{"1_to_2s":2,"over_2s":1} } }
```
